<template>
  <common-layout>
    <div class="top">
      <div class="header">
        <img alt="logo" class="logo" src="@/assets/img/logo.png">
        <span class="title">{{ systemName }}</span>
      </div>
      <div class="desc">Ant Design 是西湖区最具影响力的 Web 设计规范</div>
    </div>
    <div class="login">
      <a-form :form="form" @submit="onSubmit">
        <a-tabs
          size="large"
          :tab-bar-style="{ textAlign: 'center' }"
          style="padding: 0 2px"
        >
          <a-tab-pane key="1" tab="账户密码登录">
            <a-alert
              v-show="error"
              type="error"
              :closable="true"
              :message="error"
              show-icon
              style="margin-bottom: 24px"
            />
            <a-form-item>
              <a-input
                v-decorator="[
                  'name',
                  {
                    rules: [
                      {
                        required: true,
                        message: '请输入账户名',
                        whitespace: true,
                      },
                    ],
                  },
                ]"
                autocomplete="autocomplete"
                size="large"
                placeholder="admin"
              >
                <a-icon slot="prefix" type="user" />
              </a-input>
            </a-form-item>
            <a-form-item>
              <a-input
                v-decorator="[
                  'password',
                  {
                    rules: [
                      {
                        required: true,
                        message: '请输入密码',
                        whitespace: true,
                      },
                    ],
                  },
                ]"
                size="large"
                placeholder="888888"
                autocomplete="autocomplete"
                type="password"
              >
                <a-icon slot="prefix" type="lock" />
              </a-input>
            </a-form-item>
          </a-tab-pane>
          <a-tab-pane key="2" tab="手机号登录">
            <a-form-item>
              <a-input size="large" placeholder="mobile number">
                <a-icon slot="prefix" type="mobile" />
              </a-input>
            </a-form-item>
            <a-form-item>
              <a-row :gutter="8" style="margin: 0 -4px">
                <a-col :span="16">
                  <a-input size="large" placeholder="captcha">
                    <a-icon slot="prefix" type="mail" />
                  </a-input>
                </a-col>
                <a-col :span="8" style="padding-left: 4px">
                  <a-button
                    style="width: 100%"
                    class="captcha-button"
                    size="large"
                  >获取验证码</a-button>
                </a-col>
              </a-row>
            </a-form-item>
          </a-tab-pane>
        </a-tabs>
        <div>
          <a-checkbox :checked="true">自动登录</a-checkbox>
          <a style="float: right">忘记密码</a>
        </div>
        <a-form-item>
          <a-button
            :loading="logging"
            style="width: 100%; margin-top: 24px"
            size="large"
            html-type="submit"
            type="primary"
          >登录</a-button>
        </a-form-item>
        <div>
          其他登录方式
          <a-icon class="icon" type="alipay-circle" />
          <a-icon class="icon" type="taobao-circle" />
          <a-icon class="icon" type="weibo-circle" />
          <router-link
            style="float: right"
            to="/dashboard/workplace"
          >注册账户</router-link>
        </div>
      </a-form>
    </div>
  </common-layout>
</template>

<script>
import CommonLayout from '@/layouts/CommonLayout'
import Cookie from 'js-cookie'
import { login, getInfo } from '@/services/httpapi'
import { loadRoutes } from '@/utils/routerUtil'
import { mapMutations } from 'vuex'
import crypto from '@/utils/crypto.js'

export default {
  name: 'Login',
  components: { CommonLayout },
  data() {
    return {
      logging: false,
      error: '',
      form: this.$form.createForm(this)
    }
  },
  computed: {
    systemName() {
      return this.$store.state.setting.systemName
    }
  },
  methods: {
    ...mapMutations('account', ['setUser', 'setPermissions', 'setRoles']),
    onSubmit(e) {
      e.preventDefault()
      this.form.validateFields((err) => {
        if (!err) {
          this.doSubmit()
        }
      })
    },
    doSubmit() {
      this.logging = true
      const name = this.form.getFieldValue('name')
      const password1 = this.form.getFieldValue('password')
      const password = crypto.MD5(name + password1)
      login(name, password).then(this.afterLogin)
    },
    afterLogin(res) {
      this.logging = false
      if (res.code >= 0) {
        localStorage['token'] = res.data
        getInfo().then(this.getInfo)
        this.$message.success(res.message, 3)
      } else {
        this.error = res.message
      }
    },
    getInfo(result) {
      const data = result.data
      this.setUser({ userId: data.userId,
        account: data.account,
        userStatus: data.userStatus,
        username: data.username,
        deptName: data.deptName,
        deptType: data.deptType,
        email: data.email,
        facephoto: data.facephoto,
        lastLoginTime: data.lastLoginTime,
        orgId: data.orgId,
        password: data.password,
        phone: data.phone,
        realName: data.realName,
        sex: data.sex,
        nginxPath: data.nginxPath,
        dictList: data.dictList
      })
      this.setPermissions({ })
      this.setRoles({
        roleIds: data.roleList,
        roleType: data.roleType
      })
      this.processRoute(data.menuList)
      Cookie.set('Authorization', 'true')
      // 获取路由配置
      loadRoutes(data.menuList)
      this.$router.push('/home')
    },
    processRoute(menus) {
      const filter = (f) => {
        if (typeof (f.component) === 'string') {
          if (f.component === 'layout/Layout') {
            f.component = () => '@/layouts/PageView'
            // 添加权限
            f.meta && !f.meta.authority && (f.meta['authority'] = { permission: '*' })
          }
          if (!f.fullPath) f.fullPath = '/'
          try {
            // console.log('load route f.component module, try ', f.component)
            require(f.component) // 仅探测模块是否存在, 不存在出异常
            f.component = () => import(f.component) // 模块存在直接加载
            // console.log('load route ok ')
          } catch (error) {
            // console.log('load route f.component error, uing /layouts/StandsView', f.component)
            f.component = () => import('@/pages/list/CardList')
            !f.children && (f.redirect = '/pages/list/CardList')
          }
        }
      }
      const allNodes = (nodes) => {
        for (const node of nodes) {
          filter(node)
          node.children && allNodes(node.children)
        }
      }
      allNodes(menus)
    }
  }
}
</script>

<style lang="less" scoped>
.common-layout {
  .top {
    text-align: center;
    .header {
      height: 44px;
      line-height: 44px;
      a {
        text-decoration: none;
      }
      .logo {
        height: 44px;
        vertical-align: top;
        margin-right: 16px;
      }
      .title {
        font-size: 33px;
        color: @title-color;
        font-family: 'Myriad Pro', 'Helvetica Neue', Arial, Helvetica,
          sans-serif;
        font-weight: 600;
        position: relative;
        top: 2px;
      }
    }
    .desc {
      font-size: 14px;
      color: @text-color-second;
      margin-top: 12px;
      margin-bottom: 40px;
    }
  }
  .login {
    width: 368px;
    margin: 0 auto;
    @media screen and (max-width: 576px) {
      width: 95%;
    }
    @media screen and (max-width: 320px) {
      .captcha-button {
        font-size: 14px;
      }
    }
    .icon {
      font-size: 24px;
      color: @text-color-second;
      margin-left: 16px;
      vertical-align: middle;
      cursor: pointer;
      transition: color 0.3s;

      &:hover {
        color: @primary-color;
      }
    }
  }
}
</style>
