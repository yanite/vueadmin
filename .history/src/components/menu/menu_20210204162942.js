...
export default {
  ...
  methods: {
    ...
    renderMenuItem: function(h, menu) {
      ...
      console.log(`render ${menu.name} full path ${menu.fullpath}`)
      if (menu.name === '首页') debugger
      return h(Item, { key: 'idx_' + this_.idx++ }, [
        h(tag, config, [
          this.renderIcon(
            h,
    ...
    renderSubMenu: function(h, menu) {
      ...
      const itemArr = []
      console.log(`render ${menu.name}`)
      if (menu.name === '首页') debugger
      menu.children.forEach(function(item) {
        itemArr.push(this_.renderItem(h, item))
      })
      return h(SubMenu, { key: 'idx' + this_.idx++ }, subItem.concat(itemArr))
    },
    renderItem: function(h, menu) {
      ...
      if (!meta || !meta.invisible) {
        ...
        return menu.children && renderChildren ? this.renderSubMenu(h, menu)
    ...
    updateMenu() {
      ...
      if (!fastEqual(openKeys, this.sOpenKeys)) {
        this.collapsed || this.mode === 'horizontal' ? (this.cachedOpenKeys = openKeys)
